...
THEMES = [
    (
        "Boss takes credit for my multi-million dollar software build in front"
        " of the CEO"
    ),
    (
        "Business partner secretly signs an exclusive deal to cut me out of our"
        " startup"
    ),
    (
        "Co-worker tries to get me fired by tampering with an executive audit"
        " report"
    ),
    "Company owner denies my promised equity right before an acquisition",
    (
        "Manager blocks my transfer while demanding I train their unqualified"
        " relative"
    ),
]
...
def generate_scripts() -> List[Dict]:
  ...
  for i, theme in enumerate(THEMES, start=1):
    ...
    logger.info(
        "Generating script %d/%d for theme: %s", i, len(THEMES), theme
    )
    for attempt in range(1, GENERATION_RETRIES + 1):
      try:
        ...
        logger.exception(
            "Generation attempt %d failed for theme %s", attempt, theme
        )
        if attempt == GENERATION_RETRIES:
          raise RuntimeError(
              f"Failed to generate script for theme '{theme}'"
          ) from exc
  return scripts
...
def build_email_message(
  ...
  email_body_text = (
      f"Here are your {len(scripts)} daily scripts and audio voiceovers for"
      f" {today_str}:\n\n"
  )
  for item in scripts:
    idx = item["index"]
    email_body_text += (
        f"=== SCRIPT {idx}: {item['theme'].upper()} ===\n{item['text']}\n\n"
        + ("-" * 40)
        + "\n\n"
    )
...
async def main():
  ...
  try:
    for item in scripts:
      tmp = Path(
          tempfile.mkstemp(prefix=f"story_{item['index']}_", suffix=".mp3")[1]
      )
